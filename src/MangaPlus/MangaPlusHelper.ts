...
export class TitleDetailView {
  ...
  private get isOneShot(): boolean {
    return (
      ...
      this.firstChapterList
        .at(0)
        ?.name?.localeCompare("one-shot", undefined, { sensitivity: "base" }) ==
        0
    );
  }
  ...
  private get isReEdition(): boolean {
    return (
      this.viewingPeriodDescription?.search(TitleDetailView.REEDITION_REGEX) !=
      0
    );
  }
  ...
  private get isCompleted(): boolean {
    return (
      this.nonAppearanceInfo?.search(TitleDetailView.COMPLETED_REGEX) != 0 ||
      this.isOneShot
    );
  }
