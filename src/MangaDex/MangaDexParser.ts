...
export const parseMangaList = async (object: MangaItem[], COVER_BASE_URL: string, thumbnailSelector: () => string): Promise<MangaItemWithAdditionalInfo[]> => {
    const results: MangaItemWithAdditionalInfo[] = []

    for (const manga of object) {
        const mangaId = manga.id
        const mangaDetails = manga.attributes
        const title = Application.decodeHTMLEntities(mangaDetails.title.en ?? mangaDetails.altTitles.map(x => Object.values(x).find((v) => v !== undefined)).find((t) => t !== undefined))
        const coverFileName = manga.relationships.filter((x) => x.type == 'cover_art').map((x) => x.attributes?.fileName)[0]
        const image = coverFileName ? `${COVER_BASE_URL}/${mangaId}/${coverFileName}${MDImageQuality.getEnding(thumbnailSelector())}` : 'https://mangadex.org/_nuxt/img/cover-placeholder.d12c3c5.jpg'
...
export const parseMangaDetails = (mangaId: string, COVER_BASE_URL: string, json: any): SourceManga => {
    const mangaDetails = json.data.attributes

    const secondaryTitles: string[] = mangaDetails.altTitles.flatMap((x: any) => Object.values(x)).map((x: string) => Application.decodeHTMLEntities(x))
    const primaryTitle: string = mangaDetails.title['en'] ?? Object.values(mangaDetails.title)[0]
    const desc = Application.decodeHTMLEntities(mangaDetails.description.en)?.replace(/\[\/?[bus]]/g, '') // Get rid of BBcode tags
